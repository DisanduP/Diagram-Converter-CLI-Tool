...
function calculatePositions(nodes, direction, edges) {
  ...
  function positionNode(nodeId, startCol) {
    ...
    // Special handling for decision nodes (nodes with exactly 2 children)
    // Position them side-by-side instead of sequentially
    if (children.length === 2) {
      const leftChild = children[0];
      const rightChild = children[1];
      
      // Position left child at current column
      if (!positioned.has(leftChild)) {
        nodeCol.set(leftChild, startCol);
        positioned.add(leftChild);
        // Recursively position left child's children
        positionNode(leftChild, startCol);
      }
      
      // Position right child at next column
      if (!positioned.has(rightChild)) {
        nodeCol.set(rightChild, startCol + 1);
        positioned.add(rightChild);
        // Recursively position right child's children
        positionNode(rightChild, startCol + 1);
      }
      
      // Center parent over the two children
      nodeCol.set(nodeId, startCol + 0.5);
      return startCol + 2;
    }
