...
function parseERDiagram(lines) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const relMatch = line.match(/(\w+)\s*(\|\||o\||\|o|o\{|\}\||\|{)\s*--\s*(\|\||o\||\|o|o\{|\}\||\{o)\s*(\w+)\s*:\s*"?([^"]+)"?/);
    if (relMatch) {
    ...
    // Entity attribute: ENTITY { type attribute }
    const attrMatch = line.match(/(\w+)\s*\{\s*(\w+)\s+(\w+)/);
    if (attrMatch) {
      const [, entityName, attrType, attrName] = attrMatch;
